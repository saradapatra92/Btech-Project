...
[t,X]=ode45('TwoLink2',tspan,x0);

figure(1)
plot(t,X(:,1),'b',t,X(:,2),'r'), grid on % time domain plots
ylabel('\theta angles')
xlabel('Time (seconds)')
...
subplot(2,1,2)
plot(t,X(:,4),'r'), grid on % time domain plots
ylabel('(d/dt) \theta2')
xlabel('time')
...
xlabel(' \theta1')
subplot(2,1,2)
plot(X(:,2),X(:,4),'r'), grid on %Phase Plane plot
ylabel('(d/dt) \theta2')
xlabel('\theta2')
...
figure(6)
hold on
plot(t,X(:,5),'b') % time domain plots
